...
def obtener_nadir_altitud_camara_optimized(nasa_id):
    """ SCRAPING DE NADIR, ALTITUD Y CÁMARA - IGUAL QUE EN descargarAhora()"""
    MAX_RETRIES = 2
    ...
    for intento in range(MAX_RETRIES + 1):
        try:
            ...
            try:
                fecha_elements = soup.find_all(
                    "td", string=re.compile(r"Date taken", re.I)
                )
                if fecha_elements:
                    fecha_cell = fecha_elements[0].find_next_sibling("td")
                    if fecha_cell:
                        date_text = fecha_cell.get_text(strip=True)
                        # Convertir 2025.03.18 a date object
                        fecha_captura = datetime.strptime(date_text, "%Y.%m.%d").date()
            except Exception as e:
                log_custom(f" Error extrayendo fecha para {nasa_id}: {e}", "ERROR")

            # SELECTOR CORREGIDO PARA CÁMARA (comentado por ahora, usar DB)
            ...
            #  BUSCAR ALTITUD
            alt_value = None
            altitude_text = response.text
            alt_match = re.search(
                r"Spacecraft Altitude[^(]*\(([\d.,]+)km\)", altitude_text
            )
            if alt_match:
            ...
            #  GUARDAR EN CACHE
            with cache_lock:
                nadir_alt_cache[nasa_id] = resultado

            log_custom(
                section="Scraping NASA",
                message=f"Datos obtenidos para {nasa_id}: Cámara={camera_text}, Fecha={fecha_captura}, GeoTIFF={has_geotiff}",
                level="INFO",
                file=LOG_FILE,
            )
            ...
            log_custom(
                section="Scraping NASA",
                message=f"Intento {intento + 1} fallido para {nasa_id}: {str(e)}",
...
def obtener_camera_metadata_optimized(nasa_id):
    """ DESCARGAR CAMERA METADATA - IGUAL QUE EN descargarAhora()"""
    MAX_RETRIES = 2
    ...
    for intento in range(MAX_RETRIES + 1):
        try:
            ...
            log_custom(
                section="Camera Metadata",
                message=f"Error en camera metadata (intento {intento + 1}) para {nasa_id}: {str(e)}",
                level="WARNING" if intento < MAX_RETRIES else "ERROR",
                file=LOG_FILE,
            )
...
def get_output_folder():
    ...
    try:
        ...
        # Valores por defecto si no se puede importar rutas
        NAS_PATH = "/mnt/nas"
        NAS_MOUNT = "/mnt/nas"
        log_custom(
            section="Camera Metadata",
            message="No se pudo importar rutas, usando valores por defecto",
            level="WARNING",
            file=LOG_FILE,
        )
    ...
    try:
        ...
        log_custom(
            section="Camera Metadata",
            message=f"Error creando carpeta {camera_data_path}: {str(e)}",
            level="ERROR",
            file=LOG_FILE,
        )
...
def extract_metadatos_enriquecido(results):
    """ EXTRACCIÓN DE METADATOS CON SCRAPING COMPLETO - IGUAL QUE descargarAhora()"""
    ...
    log_custom(
        ...
        message=f"Extrayendo metadatos enriquecidos de {len(results)} resultados con scraping",
        level="INFO",
        file=LOG_FILE,
    )
    ...
    try:
        ...
        log_custom(
            section="Extracción Metadatos",
            message=f"Error cargando data.py: {str(e)}. Verifica que el archivo existe en utils/",
            level="ERROR",
            file=LOG_FILE,
        )
        # Sin mapeos por defecto - el proceso fallará si no se puede cargar data.py
        raise ImportError("No se pudo cargar data.py - proceso detenido")

    def procesar_imagen_con_scraping(photo):
        """Procesar una imagen individual con scraping completo"""
        try:
            ...
            #  HACER SCRAPING COMPLETO - IGUAL QUE EN descargarAhora()
            ...
            metadata_completo = {
                "NASA_ID": nasa_id,
                "FECHA": fecha_final,  #  FECHA REAL DE CAPTURA
                "HORA": formatted_hour,
            ...
            log_custom(
                section="Extracción Metadatos",
                message=f"Error procesando imagen con scraping: {str(e)}",
                level="ERROR",
                file=LOG_FILE,
            )
            return None
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        future_to_photo = {
            executor.submit(procesar_imagen_con_scraping, photo): photo
        ...
        for future in as_completed(future_to_photo):
            try:
                ...
                if resultado:
                    metadatos_enriquecidos.append(resultado)
                ...
                if procesadas % 10 == 0 or procesadas == len(results):
                    log_custom(
                        section="Extracción Metadatos Enriquecida",
                        message=f"Progreso scraping: {procesadas}/{len(results)} completadas",
                        level="INFO",
                        file=LOG_FILE,
                    )
    ...
    log_custom(
        section="Extracción Metadatos Enriquecida",
        message=f"Metadatos enriquecidos extraídos: {len(metadatos_enriquecidos)} registros con scraping completo",
        level="INFO",
        file=LOG_FILE,
    )
