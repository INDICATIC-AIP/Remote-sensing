// Renderer with optimized logging and fixed download time formatting
  ...
  .then(template => {
      ...
      methods: {
        ...
        async obtenerNadirAltitudCamaraOptimized(nasaId) {
          ...
          for (let intento = 0; intento <= MAX_RETRIES; intento++) {
            try {
              ...
              // Check for GeoTIFF availability
              ...
              console.log(`Data for ${nasaId}: Camera=${cameraText}, Nadir=${nadirText}, Alt=${altValue}, Date=${fechaCaptura}, GeoTIFF=${hasGeotiff ? 'YES' : 'NO'}`);
              ...
              console.warn(`Attempt ${intento + 1} failed for ${nasaId}: ${err.message}`);

              if (intento === MAX_RETRIES) {
                ipcRenderer.send("log_custom", {
                  message: `Error obtaining NADIR/ALTITUDE/DATE for ${nasaId} after ${MAX_RETRIES + 1} attempts: ${err.message}`,
                  level: "ERROR",
                  file: LOG_PATH
                });
                return {
            ...
            try {
              ...
              if (button.length && button.attr("onclick")) {
                ...
                if (fileUrl && fileUrl.startsWith("/")) {
                  ...
                  // Reuse existing file when present and valid
              ...
              console.log(`No camera metadata button found for ${nasaId}`);
              return null;

            } catch (err) {
              console.warn(`Error in camera metadata (attempt ${intento + 1}) for ${nasaId}: ${err.message}`);

              if (intento === MAX_RETRIES) {
                ipcRenderer.send("log_custom", {
                  message: `Error in CAMERA_METADATA for ${nasaId} after ${MAX_RETRIES + 1} attempts: ${err.message}`,
                  level: "ERROR",
                  file: LOG_PATH
                });
                return null;
        ...
        deduplicarMetadatos(datos) {
          ...
          for (const item of datos) {
            ...
            if (!vistos.has(nasaId)) {
              ...
              duplicados++;
              // console.warn(`Duplicate detected and removed: ${nasaId}`);
            }
          }

          //  LOGGING INTELIGENTE según el contexto
          const contexto = datos.length > 0 && datos[0].NASA_ID ? "Metadata" : "API Results";

          ipcRenderer.send("log_custom", {
            section: `Deduplication ${contexto}`,
            message: `Unique elements: ${unicos.length}, duplicates removed: ${duplicados}`,
            level: "INFO",
            file: LOG_PATH
          });
        ...
        async descargarAhora() {
          ...
          if (this.todasEnBD || this.resultadosNuevos.length === 0) {
            alert("No new images to download. All are already in the database.");
            return;
          }
          ...
          if (this.limiteDescarga > 0 && this.limiteDescarga < this.resultadosNuevos.length) {
            ...
            console.log(`DOWNLOAD: Applying limit - processing ${imagenesToProcess.length} of ${this.resultadosNuevos.length} new images`);
          } else {
            imagenesToProcess = [...this.resultadosNuevos];
            console.log(`DOWNLOAD: No limit - processing all ${imagenesToProcess.length} new images`);
          }

          const outputPath = path.join(__dirname, "..", "periodic_metadata.json");
          ...
          this.mensajeEstado = `Processing ${imagenesToProcess.length} new images...`;
          ...
          ipcRenderer.send("log_custom", {
            section: "Manual Download Start",
            message: `Starting manual download of ${imagenesToProcess.length} new images`,
            level: "INFO",
            file: LOG_PATH
          });
          ...
          const processPhotoOptimized = async (photo) => {
            try {
              ...
              if (!filename) {
                console.warn("Missing filename:", photo);
                return null;
              }
              ...
              const cameraCode = findBySuffix(photo, ".camera", "Unknown");
              const cameraDesc = cameraMap[cameraCode] || "Unknown";

              const filmCode = findBySuffix(photo, ".film", "UNKN");
              const filmData = filmMap[filmCode] || { type: "Unknown", description: "Unknown" };

              const nasaId = filename?.split(".")[0] || "NO_ID";

              if (!nasaId || nasaId === "NO_ID") {
                console.warn("Invalid NASA_ID:", nasaId, "filename:", filename);
                return null;
              }
              ...
              if (cameraDesc.includes("Desconocida") || cameraDesc.includes("Desconocido") || cameraDesc.includes("Unspecified : ")) {
                camera = extraData?.CAMARA || "Unknown";
              } else {
                camera = cameraDesc;
              }
              ...
              console.log(`Processed: ${nasaId} - URL: ${finalImageUrl ? 'YES' : 'NO'} - Date: ${fechaFinal}`);
              return resultado;
          ...
          try {
            ...
            this.estadoDescarga = 'processing';
            this.mensajeEstado = 'Processing image metadata...';

            ipcRenderer.send("log_custom", {
              section: "Phase 1 - Processing",
              message: `PHASE 1: Processing ${imagenesToProcess.length} metadata entries with optimized parallelism`,
              level: "INFO",
              file: LOG_PATH
            });
            ...
            const processWithConcurrencyLimit = async (items, processor, limit) => {
              ...
              for (let i = 0; i < items.length; i += limit) {
                ...
                console.log(`Processing batch ${batchNum}/${totalBatches}: items ${i} to ${Math.min(i + limit - 1, items.length - 1)}`);

                // Update status message with detailed progress
                this.mensajeEstado = `Processing metadata... Batch ${batchNum} of ${totalBatches} (${Math.floor((procesados / items.length) * 100)}%)`;
                ...
                batchResults.forEach((result, index) => {
                  if (result.status === 'fulfilled') {
                    ...
                    if (data && data.NASA_ID) {
                      results.push(data);
                      console.log(`Processed successfully: ${data.NASA_ID}`);
                    } else {
                      console.log(`Result without valid NASA_ID:`, data);
                    }
                  } else {
                ...
                if (batchNum % 5 === 0 || procesados >= items.length) {
                  ipcRenderer.send("log_custom", {
                    message: `Processed ${procesados} of ${items.length} metadata (${Math.floor(progresoFase1 * 100)}%) - Batch ${batchNum}/${totalBatches}`,
                    level: "INFO",
                    file: LOG_PATH
                  });
                }
            ...
            console.log(`PHASE 1 SUMMARY (BEFORE deduplication): ${metadatos.length} metadata processed from ${imagenesToProcess.length} attempted`);

            //  APLICAR DEDUPLICACIÓN
            this.mensajeEstado = 'Eliminando duplicados...';
            const metadatosUnicos = this.deduplicarMetadatos(metadatos);

            console.log(`PHASE 1 SUMMARY (AFTER deduplication): ${metadatosUnicos.length} unique metadata`);

            //  GUARDAR JSON CON METADATOS ÚNICOS
            if (metadatosUnicos.length > 0) {
              this.mensajeEstado = 'Saving unique metadata...';

              fs.writeFileSync(outputPath, JSON.stringify(metadatosUnicos, null, 2), "utf-8");

              ipcRenderer.send("log_custom", {
                message: `PHASE 1 COMPLETE: JSON saved with ${metadatosUnicos.length} unique entries (removed ${metadatos.length - metadatosUnicos.length} duplicates)`,
                level: "INFO",
                file: LOG_PATH
              });

              console.log(`JSON saved successfully with unique metadata: ${outputPath}`);
              console.log(`First 3 unique metadata:`, metadatosUnicos.slice(0, 3));
            } else {
              throw new Error("No valid unique metadata generated to save");
            }

            //  FASE 2: DESCARGA REAL
            this.estadoDescarga = 'downloading';
            this.mensajeEstado = 'Starting image download...';

            ipcRenderer.send("log_custom", {
              message: `PHASE 2: Starting download of ${metadatosUnicos.length} unique images`,
              level: "INFO",
              file: LOG_PATH
            });
            ...
            const progressListener = (event, progressData) => {
              ...
              if (typeof progressData === 'object' && progressData.porcentaje !== undefined) {
                ...
                this.mensajeEstado = `Downloading images... ${porcentaje}% (${descargadas}/${total})`;
                ...
                if (porcentaje % 25 === 0 || porcentaje === 100) {
                  ipcRenderer.send("log_custom", {
                    message: `Download progress: ${porcentaje}% (${descargadas}/${total} images)`,
                    level: "INFO",
                    file: LOG_PATH
                  });
                }
                ...
                this.mensajeEstado = `Downloading images... ${porcentaje}%`;
                NProgress.set(progresoTotalReal);
                this.progresoDescarga = Math.floor(progresoTotalReal * 100);
            ...
            const completeListener = async () => {
              ...
              this.mensajeEstado = 'Download completed successfully';
              ...
              ipcRenderer.send("log_custom", {
                section: "Download Completed",
                message: `PROCESS COMPLETED: ${metadatos.length} metadata processed and download finished in ${this.tiempoDescargaFormato}`,
                level: "INFO",
                file: LOG_PATH
              });
              ...
              try {
                ipcRenderer.send("log_custom", {
                  section: "Automatic Reload",
                  message: "Starting automatic reload of results",
                  level: "INFO",
                  file: LOG_PATH
                });
                ...
                ipcRenderer.send("log_custom", {
                  ...
                  message: "Result list refreshed successfully",
                  level: "INFO",
                  file: LOG_PATH
                });
                ...
                ipcRenderer.send("log_custom", {
                  section: "Reload Error",
                  message: `Error reloading list: ${error.message}`,
                  level: "ERROR",
                  file: LOG_PATH
                });

                console.error('Error reloading list:', error);
                alert(`Download completed, but there was an error reloading the list: ${error.message}`);
              }
            };
            ...
            console.log('Invoking download from Python...');
            const response = await ipcRenderer.invoke("descargarDirecto");

            console.log("Download invocation completed:", response);
            ...
            ipcRenderer.send("log_custom", {
              message: `Error during manual download: ${err.message}`,
              level: "ERROR",
              file: LOG_PATH
            });
        ...
        getOutputFolder() {
          ...
          const nasPath = "/mnt/nas";  // Keep in sync with routes.py
          ...
          // Mirror NAS check logic from Python
          ...
          const modo = useNas ? "PRODUCTION (NAS)" : "DEVELOPMENT (Local - tests only)";
          ...
          try {
            ...
            ipcRenderer.send("log_custom", {
              ...
              message: `Could not create directory at ${finalPath}: ${err.message}`,
              level: "ERROR",
              file: LOG_PATH
            });
          }
        ...
        async fetchData(esRecargaAutomatica = false) {
          ...
          if (!this.allFiltersValid) {
            alert("All filters must be completed and contain valid characters.");
            return;
          }

          const seccionLog = esRecargaAutomatica ? "Automatic Reload" : "Manual Search";
          const mensajeLog = esRecargaAutomatica
            ? "Reloading images after download"
        ...
        async procesarConsulta(apiUrl, source, allResults) {
          try {
            ...
            if (!Array.isArray(rawData)) {
              ipcRenderer.send("log_custom", {
                message: `No results found for source: ${source}`,
                level: "WARNING",
                file: LOG_PATH
              });
              return;
            ...
            ipcRenderer.send("log_custom", {
              message: `Query error for ${source}: ${err.message}`,
              level: "ERROR",
              file: LOG_PATH
            });
          }
        ...
        selectImage(photo) {
          if (!this.map || !this.clusterGroup) {
            ipcRenderer.send("log_custom", {
              message: "Map or clusterGroup not initialized",
              level: "WARNING",
              file: LOG_PATH
            });
            return;
          ...
          if (!isNaN(lat) && !isNaN(lon)) {
            ...
            if (markerEntry) {
              ...
              ipcRenderer.send("log_custom", {
                message: "Could not find marker to open",
                level: "WARNING",
                file: LOG_PATH
              });
            }
        ...
        actualizarBoundingBox(nuevoBox) {
          ...
          ipcRenderer.send("log_custom", {
            message: `Bounding box updated and filters applied. Lat: ${this.boundingBox.latMin}-${this.boundingBox.latMax}, Lon: ${this.boundingBox.lonMin}-${this.boundingBox.lonMax}`,
            level: "INFO",
            file: LOG_PATH
          });
        },
        ...
        convertirUTCaLocal(ptimeUTC) {
          ...
          if (horasLocal >= 6 && horasLocal < 12) {
            periodoTexto = "in the morning";
          } else if (horasLocal >= 12 && horasLocal < 19) {
            periodoTexto = "in the afternoon";
          } else {
            periodoTexto = "at night";
          }
          ...
          return `Local Time: ${horasStr}:${minutosLocal}:${segundosLocal} ${periodoTexto}`;
        },
        ...
        actualizarTareasDesdeArchivo() {
          ...
          try {
            ...
            ipcRenderer.send("log_custom", {
              message: `Error reading tasks: ${err.message}`,
              level: "ERROR",
              file: LOG_PATH
            });
            this.tareasPeriodicas = [];
        ...
        validarCron() {
          if (!this.cronHora || this.cronHora.trim() === "") {
            alert("You must specify the start time.");
            return false;
          }
          ...
          if (
            ...
            alert("Enter a valid interval (greater than 0).");
            return false;
          }
        ...
        cancelarDescarga() {
          if (confirm('Are you sure you want to cancel the download?')) {
            ...
            // Log de cancelación
            ipcRenderer.send("log_custom", {
              section: "Download Cancelled",
              message: "User cancelled the download manually",
              level: "WARNING",
              file: LOG_PATH
            });
            ...
            setTimeout(async () => {
              try {
                await this.fetchData(true); // Automatic reload
                alert('Download cancelled. List refreshed to show current state.');
              } catch (error) {
        ...
        <div class="notification-content">
            ...
            <div class="notification-text">
                ...
                <p>${cantidadProcesada} images processed successfully</p>
                <p>List refreshed automatically</p>
            </div>
    ...
    `;
        ...
        async verificarNasaIdsEnBD() {
          ...
          ipcRenderer.send("log_custom", {
            section: "NASA_ID Verification",
            message: `Verifying ${this.results.length} NASA_IDs against database`,
            level: "INFO",
            file: LOG_PATH
          });
          ...
          try {
            const verificacionPromises = this.results.map(async (photo) => {
              ...
              if (!nasaId) {
                console.warn(`Result without valid filename:`, photo);
                return null;
              }
              ...
              try {
                ...
                return null; // Ya existe en BD o no es alta resolución
                  ...
                  ipcRenderer.send("log_custom", {
                    message: `Error verifying NASA_ID ${nasaId}: ${err.message}`,
                    level: "ERROR",
                    file: LOG_PATH
                  });
                return null;
            ...
            if (this.resultadosNuevos.length === 0) {
              ...
              ipcRenderer.send("log_custom", {
                ...
                message: "All images are already in the database",
                level: "INFO",
                file: LOG_PATH
              });
            } else {
              ...
              ipcRenderer.send("log_custom", {
                section: "Verification Completed",
                message: `${this.resultadosNuevos.length} new images found of ${this.results.length} total`,
                level: "INFO",
                file: LOG_PATH
              });
            }
            ...
            ipcRenderer.send("log_custom", {
              ...
              message: `Error during NASA_ID verification: ${error.message}`,
              level: "ERROR",
              file: LOG_PATH
            });
            this.resultadosNuevos = [];
        ...
        actualizarMapaConFiltros() {
          ...
          // Add only markers from filtered results
          if (this.resultadosFiltrados.length > 0) {
            ...
            ipcRenderer.send("log_custom", {
              message: `Map updated with ${this.resultadosFiltrados.length} markers (filter: ${this.vistaFiltro})`,
              level: "INFO",
              file: LOG_PATH
            });
          }
